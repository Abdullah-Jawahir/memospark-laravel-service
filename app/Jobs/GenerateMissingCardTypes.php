...
class GenerateMissingCardTypes implements ShouldQueue
{
    ...
    public function handle(FastApiService $fastApiService)
    {
        try {
            ...
            if (!$document || empty($this->missingTypes)) {
                Log::channel('fastapi')->info('GenerateMissingCardTypes: Document not found or no missing types', [
                    'document_id' => $this->documentId,
                    'missing_types' => $this->missingTypes
                ]);
                return;
            }
            ...
            if (!file_exists($this->filePath)) {
                Log::channel('fastapi')->warning('GenerateMissingCardTypes: File not found', [
                    'document_id' => $this->documentId,
            ...
            Log::channel('fastapi')->error('GenerateMissingCardTypes job failed', [
                'document_id' => $this->documentId,
                'missing_types' => $this->missingTypes,
                'error' => $e->getMessage(),
    ...
    {
        Log::channel('fastapi')->error('GenerateMissingCardTypes job permanently failed', [
            'document_id' => $this->documentId,
            'missing_types' => $this->missingTypes,
